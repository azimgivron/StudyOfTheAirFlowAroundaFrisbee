...
Solving this problem numerically can be done by using different methods\footnote{(Schroeder 2015, 1-32)}. Here, we will use the Euler progressive method. It simply means that position is calculated from the previous one\footnote{(Morrison V. R. 2005, 1-12)}.
...
\section{Results}
In this section, we will present three graphs\footnote{(Baumback et al. 2010, 1-16)} that we obtained from the simulation. All of them were made with the same parameters values except for the angle of attack which is varying from $5\,^{\circ}$ up to $45\,^{\circ} $. 
